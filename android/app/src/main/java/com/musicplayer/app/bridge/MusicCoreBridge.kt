...
object MusicCoreBridge {
    ...
    // 5.1 修复并发安全：placeholderSources 改用 CopyOnWriteArrayList，遍历与修改可在多协程下安全进行。
    private val placeholderSources = java.util.concurrent.CopyOnWriteArrayList(
        listOf(
    ...
    // 5.2 修复并发安全：placeholderSeq 改用 AtomicInteger，避免多协程同时自增导致重复 ID。
    private val placeholderSeq = java.util.concurrent.atomic.AtomicInteger(100)
    ...
    suspend fun importSourceFromJson(jsonStr: String): SourceInfo = withContext(Dispatchers.IO) {
        ...
        // 5.2 使用 AtomicInteger.getAndIncrement() 保证 ID 唯一。
        val n = placeholderSeq.getAndIncrement()
        val info = SourceInfo(
            id = "src_json_$n",
            name = "导入音源 ${n - 100}",
            version = "1.0.0",
