...
- 实现本地音乐播放：扫描本地音乐目录、解析音频元数据、构建本地音乐库、文件夹监听增量更新，并与搜索/播放列表/收藏夹/播放组件统一对接
...
- Affected code: 全新代码库
  ...
  - `core/sources/local/`（本地音乐源：目录扫描、元数据解析、库索引、文件监听）
...
### Requirement: 本地音乐播放
系统 SHALL 支持本地音乐播放：用户可添加本地音乐目录，系统扫描其中音频文件并解析内嵌元数据（ID3/FLAC/MP4/APE 等标签），构建本地音乐库；支持文件夹监听做增量更新；本地音乐作为内置源接入聚合搜索、播放列表、收藏夹与播放组件，与在线音源体验一致。

#### Scenario: 添加本地目录并扫描
- **WHEN** 用户在设置内添加一个本地音乐目录
- **THEN** 系统递归扫描目录下音频文件，解析元数据并入库，展示扫描进度与结果

#### Scenario: 本地音乐元数据解析
- **WHEN** 扫描到某音频文件
- **THEN** 系统解析其内嵌标签得到标题/艺术家/专辑/封面/时长，缺失字段以文件名等合理回退值填充

#### Scenario: 文件夹增量更新
- **WHEN** 已监听的本地目录发生新增/删除/移动文件
- **THEN** 系统增量同步本地音乐库，无需全量重扫

#### Scenario: 本地音乐统一接入
- **WHEN** 用户在搜索/播放列表/收藏夹中操作
- **THEN** 本地音乐与在线音源统一呈现，结果标注来源为「本地」，可直接播放本地文件

#### Scenario: 本地音乐播放
- **WHEN** 用户播放一首本地音乐
- **THEN** 系统通过该端原生音频组件直接读取本地文件播放，无需联网与缓存
